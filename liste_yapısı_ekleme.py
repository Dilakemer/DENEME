class Magaza:
    ...
    def __str__(self):
        return f"Mağaza Adı: {self.__magaza_adi}\nSatıcı Adı: {self.__satici_adi}\nSatış Cinsi: {self.__satici_cinsi}\nTutar: {self.__tutar}\nÇalıştığı Şehir: {self.__satici_calistigi_sehir}"
def main():
    magazalar_list = []
    while True:
        ...
        magazalar_list.append(magaza_instance)

    def magaza_satis_tutar(magazalar_list, magaza_adi):
        toplam_tutar = 0
        for magaza_instance in magazalar_list:
            if magaza_instance.get_magaza_adi() == magaza_adi:
                toplam_tutar += magaza_instance.get_tutar()
        return toplam_tutar

    for magaza_instance in magazalar_list:
        toplam_tutar = magaza_satis_tutar(magazalar_list, magaza_instance.get_magaza_adi())
